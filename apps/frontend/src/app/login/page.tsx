  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="bg-white/90 dark:bg-slate-900/80 backdrop-blur-md w-full max-w-md rounded-2xl shadow-xl dark:shadow-slate-900/50 overflow-hidden border border-slate-200/50 dark:border-slate-800">
        {/* Header */}
        <div className="bg-gradient-to-r from-primary-500 to-primary-600 p-8 text-center relative overflow-hidden">
          {/* Decorative circles */}
          ...
          <h1 className="text-2xl font-bold text-white relative">Đăng nhập</h1>
          <p className="text-white/80 mt-2 text-sm relative">Chào mừng bạn quay lại hệ thống học tập</p>
        </div>

        {/* Form */}
        <div className="p-8">
          {registered && (
            <div className="mb-4 p-3 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-700 dark:text-emerald-400 rounded-xl text-sm font-medium text-center">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button disabled={loading} type="submit" className="block w-full py-3.5 px-4 bg-gradient-to-r from-primary-500 to-primary-600 hover:from-primary-600 hover:to-primary-700 text-white text-center rounded-xl font-bold transition-all hover:shadow-lg shadow-primary-500/25 disabled:opacity-50 disabled:hover:shadow-none">
          ...
          <div className="mt-6 flex items-center">
            ...
            <span className="px-4 text-sm text-slate-400 dark:text-slate-500">hoặc đăng nhập bằng</span>
            <div className="flex-1 border-t border-slate-200 dark:border-slate-700" />
          </div>
...
export default function Login() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center text-slate-400 dark:text-slate-500">Đang tải...</div>}>
      <LoginForm />
    </Suspense>
  );
}
