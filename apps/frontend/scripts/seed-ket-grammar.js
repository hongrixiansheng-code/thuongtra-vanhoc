...
async function main() {
  console.log(`--- START SEEDING KET GRAMMAR ---`);

  const dataPath = path.join(__dirname, '../../../data/ket-grammar.json');
  ...
  for (const lessonData of data.lessons) {
    ...
    // Xóa nội dung GRAMMAR cũ (nếu có)
  ...
  console.log('--- DONE SEEDING KET GRAMMAR ---');
}
