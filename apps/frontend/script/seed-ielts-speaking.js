...
async function main() {
  ...
  const cumFile = args[0] || 'ielts-speaking-cum1.json';

  console.log(`🔄 Bắt đầu nạp đề Speaking từ file: ${cumFile}`);
  ...
  for (const batch of batches) {
    ...
    const speakingList = batch.speaking;
    ...
    let addedCount = 0;
    for (const speaking of speakingList) {
      const existingContent = await prisma.lessonContent.findFirst({
        where: {
          lessonId: lesson.id,
          contentType: 'SPEAKING',
          content: { contains: `"title":"${speaking.title}"` }
        }
      });
      ...
      if (!existingContent) {
        await prisma.lessonContent.create({
          data: {
            lessonId: lesson.id,
            contentType: 'SPEAKING',
            content: JSON.stringify(speaking)
          }
        });
        addedCount++;
      }
    }
    console.log(`✅ Bài ${orderIndex} (${lesson.title}): Thêm mới ${addedCount}/${speakingList.length} đề speaking.`);
  }

  console.log('🎉 Hoàn tất nạp Speaking!');
}
