...
async function main() {
  ...
  const cumFile = args[0] || 'ielts-dialogue-cum1.json';

  console.log(`🔄 Bắt đầu nạp hội thoại từ file: ${cumFile}`);
  ...
  for (const batch of batches) {
    ...
    const dialogueList = batch.dialogue;
    ...
    for (const dialogue of dialogueList) {
      const existingContent = await prisma.lessonContent.findFirst({
        where: {
          ...
          content: { contains: `"title":"${dialogue.title}"` }
        }
      });
    ...
    console.log(`✅ Bài ${orderIndex} (${lesson.title}): Thêm mới ${addedCount}/${dialogueList.length} hội thoại.`);
  }

  console.log('🎉 Hoàn tất nạp hội thoại!');
}
