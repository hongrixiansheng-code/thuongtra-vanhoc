...
async function main() {
  ...
  const cumFile = args[0] || 'ielts-grammar-cum1.json';
  
  console.log(`🔄 Bắt đầu nạp ngữ pháp từ file: ${cumFile}`);
  ...
  for (const batch of batches) {
    ...
    const grammarList = batch.grammar;
    ...
    for (const grammar of grammarList) {
      // Kiểm tra xem ngữ pháp này đã tồn tại trong bài học chưa (dựa vào title)
      const existingContent = await prisma.lessonContent.findFirst({
        where: {
          lessonId: lesson.id,
          contentType: 'GRAMMAR',
          content: { contains: `"title":"${grammar.title}"` }
        }
      });
    ...
    console.log(`✅ Bài ${orderIndex} (${lesson.title}): Thêm mới ${addedCount}/${grammarList.length} điểm ngữ pháp.`);
  }

  console.log('🎉 Hoàn tất nạp ngữ pháp!');
}
